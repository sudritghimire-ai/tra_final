    ...
    pub open spec fn encode_len3(a: u64, b: u64, c: u64, q: int) -> int {
        a as int + (b as int) * q + (c as int) * spec_pow(q, 4)
    }
    ...
    #[verifier(external_body)]
    proof fn lemma_spec_pow_four(q: int)
        ensures spec_pow(q, 4) == q * q * q * q
    {}
            ...
            new_state as int == ramanujan_spec(prev.push(li), q, p)
    {
        ...
        assert(
            ...
            == (ramanujan_spec(prev.push(li).drop_last(), q, p)
                + (prev.push(li).last() as int
                    * (spec_pow(q, (prev.push(li).len() - 1) * (prev.push(li).len() - 1)) % p))) % p
        );
        ...
        assert(
            ...
            == (ramanujan_spec(prev, q, p)
                + (li as int
                    * (spec_pow(q, (prev.len() as int) * (prev.len() as int)) % p))) % p
        );

        assert(weight as int == spec_pow(q, (prev.len() as int) * (prev.len() as int)) % p);
        assert(term as int == (li as int * (spec_pow(q, (prev.len() as int) * (prev.len() as int)) % p)) % p);

        lemma_mod_pull_left(
            li as int * (spec_pow(q, (prev.len() as int) * (prev.len() as int)) % p),
            ramanujan_spec(prev, q, p),
            p
        );

        assert(
            (((li as int * (spec_pow(q, (prev.len() as int) * (prev.len() as int)) % p)) % p)
        ...
        if logs.len() == 0 {
            ...
            lemma_mod_range_int(
                ramanujan_spec(logs.drop_last(), q, p)
                    + (logs.last() as int * (spec_pow(q, (logs.len() - 1) * (logs.len() - 1)) % p)),
                p
            );
        }
    }
        ...
        if logs.len() == 0 {
            ...
            lemma_mod_range_int(
                ramanujan_shifted_spec(logs.drop_last(), q, p, offset)
                    + (logs.last() as int
                        * (spec_pow(q, (offset + (logs.len() - 1)) * (offset + (logs.len() - 1))) % p)),
                p
            );
        }
    }
        ...
        decreases b.len()
    {
        if b.len() == 0 {
            ...
            assert(
                ((a + b).len() - 1) * ((a + b).len() - 1)
                == (a.len() + (b.len() - 1)) * (a.len() + (b.len() - 1))
            );

            assert(ramanujan_spec(a + b, q, p)
                == (ramanujan_spec((a + b).drop_last(), q, p)
                    + (((a + b).last() as int)
                        * (spec_pow(q, ((a + b).len() - 1) * ((a + b).len() - 1)) % p))) % p);
            ...
            assert(
                spec_pow(q, ((a + b).len() - 1) * ((a + b).len() - 1)) % p
                == spec_pow(q, (a.len() + (b.len() - 1)) * (a.len() + (b.len() - 1))) % p
            );
        ...
        by {
            if n <= 0 {
                ...
                assert(i <= n - 1) by (nonlinear_arith)
                    requires
                        i < n;
                assert(i * i <= (n - 1) * (n - 1)) by (nonlinear_arith)
                    requires
                        0 <= i,
                        i <= n - 1;
                assert(sq_exp(i) == i * i);
                assert(square_degree_bound(w.len() as int) == (n - 1) * (n - 1));
            }
        }
    }
    ...
    // Final theorem:
    // distinct equal-length traces have at most (n-1)^2 bad q values.
    ...
    proof fn lemma_square_exp_nonneg(i: int)
        ...
        ensures
            i * i >= 0
    {
        assert(i * i >= 0) by (nonlinear_arith)
            requires
                i >= 0;
    }
        ...
        decreases k
    {
        if k <= 0 {
            ...
            (
                ...
                +
                diff_coeff_at(w, w2, i) * (spec_pow(q, i * i) % p)
            ) % p
        }
    }
    ...
    */
            ...
            ramanujan_spec(w, q, p) != ramanujan_spec(w2, q, p)
    {
        ...
        lemma_single_diff_poly_nonzero(coeff, q, a * a, p);
        assert(term == spec_pow(q, a * a) % p);
        assert((coeff * term) % p == (coeff * (spec_pow(q, a * a) % p)) % p);
