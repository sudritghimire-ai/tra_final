...
assert(
    ...
    */
    ...
    //     length and range consistency for TraceLock chunks
    // ======================================================
    //
    // This section models the metadata carried by each TraceLock
    // checkpoint/chunk:
    //
    ...
    // checkpoint record used by TraceLock.
    //
    ...
    // ======================================================
    //
    // This is the verifier logic used by TraceLock.
    //
...
// ======================================================
// K-Sentry App-Flow Bug Detector
// Paste this section INSIDE your existing `verus! { ... }` block,
// immediately before the final closing `}`.
//
// Design:
//   1. K-Sentry protects trace integrity.
//   2. This state machine checks whether the protected trace is valid.
//   3. Numeric event/violation codes keep the verified core small.
// ======================================================
...
pub const EV_APP_OPENED: u64             = 1;
pub const EV_LOGIN_STARTED: u64           = 2;
pub const EV_LOGIN_SUCCEEDED: u64         = 3;
pub const EV_LOGIN_FAILED: u64            = 4;
pub const EV_ASSESSMENT_STARTED: u64      = 5;
pub const EV_QUESTION_ANSWERED: u64       = 6;
pub const EV_ASSESSMENT_SUBMITTED: u64    = 7;
pub const EV_RESULT_GENERATED: u64        = 8;
pub const EV_RESULT_DISPLAYED: u64        = 9;
pub const EV_NETWORK_LOST: u64            = 10;
pub const EV_NETWORK_RESTORED: u64        = 11;
pub const EV_APP_BACKGROUNDED: u64        = 12;
pub const EV_APP_FOREGROUNDED: u64        = 13;
pub const EV_APP_CLOSED: u64              = 14;
pub const EV_CRASH: u64                   = 15;

// -------------------------
// State codes
// -------------------------

pub const ST_CLOSED: u64                  = 0;
pub const ST_LOGGED_OUT: u64              = 1;
pub const ST_LOGIN_PENDING: u64           = 2;
pub const ST_LOGGED_IN: u64               = 3;
pub const ST_ASSESSMENT: u64              = 4;
pub const ST_WAITING_RESULT: u64           = 5;
pub const ST_RESULT_READY: u64             = 6;
pub const ST_RESULT_VISIBLE: u64           = 7;
pub const ST_BACKGROUND: u64               = 8;
pub const ST_OFFLINE: u64                  = 9;
pub const ST_CRASHED: u64                  = 10;

// -------------------------
// Violation codes
// -------------------------

pub const BUG_NONE: u64                    = 0;
pub const BUG_UNKNOWN_EVENT: u64           = 1;
pub const BUG_INVALID_TRANSITION: u64      = 2;
pub const BUG_LOGIN_WITHOUT_OPEN: u64      = 3;
pub const BUG_ASSESSMENT_WITHOUT_LOGIN: u64 = 4;
pub const BUG_ANSWER_OUTSIDE_ASSESSMENT: u64 = 5;
pub const BUG_SUBMIT_OUTSIDE_ASSESSMENT: u64 = 6;
pub const BUG_DUPLICATE_SUBMIT: u64        = 7;
pub const BUG_RESULT_WITHOUT_SUBMIT: u64   = 8;
pub const BUG_DISPLAY_WITHOUT_RESULT: u64  = 9;
pub const BUG_EVENT_AFTER_CRASH: u64       = 10;
pub const BUG_CLOSE_WHILE_PENDING: u64     = 11;
pub const BUG_EMPTY_TRACE: u64             = 12;

// -------------------------
// Severity codes
// -------------------------

pub const SEV_NONE: u64                    = 0;
pub const SEV_LOW: u64                     = 1;
pub const SEV_MEDIUM: u64                  = 2;
pub const SEV_HIGH: u64                    = 3;
pub const SEV_CRITICAL: u64                = 4;

// ======================================================
// Specifications
// ======================================================

pub open spec fn valid_event_code(e: int) -> bool {
    EV_APP_OPENED as int <= e && e <= EV_CRASH as int
}

pub open spec fn valid_state_code(s: int) -> bool {
    ST_CLOSED as int <= s && s <= ST_CRASHED as int
}

pub open spec fn violation_severity_spec(v: int) -> int {
    if v == BUG_NONE as int {
        SEV_NONE as int
    } else if v == BUG_UNKNOWN_EVENT as int {
        SEV_MEDIUM as int
    } else if v == BUG_INVALID_TRANSITION as int {
        SEV_HIGH as int
    } else if v == BUG_LOGIN_WITHOUT_OPEN as int {
        SEV_HIGH as int
    } else if v == BUG_ASSESSMENT_WITHOUT_LOGIN as int {
        SEV_HIGH as int
    } else if v == BUG_ANSWER_OUTSIDE_ASSESSMENT as int {
        SEV_MEDIUM as int
    } else if v == BUG_SUBMIT_OUTSIDE_ASSESSMENT as int {
        SEV_HIGH as int
    } else if v == BUG_DUPLICATE_SUBMIT as int {
        SEV_HIGH as int
    } else if v == BUG_RESULT_WITHOUT_SUBMIT as int {
        SEV_CRITICAL as int
    } else if v == BUG_DISPLAY_WITHOUT_RESULT as int {
        SEV_HIGH as int
    } else if v == BUG_EVENT_AFTER_CRASH as int {
        SEV_CRITICAL as int
    } else if v == BUG_CLOSE_WHILE_PENDING as int {
        SEV_MEDIUM as int
    } else {
        SEV_LOW as int
    }
...
// Returns the bug code for one transition.
pub open spec fn transition_bug_spec(state: int, event: int) -> int {
    if !valid_event_code(event) {
        BUG_UNKNOWN_EVENT as int
    } else if state == ST_CRASHED as int && event != EV_APP_OPENED as int {
        BUG_EVENT_AFTER_CRASH as int
    } else if event == EV_APP_OPENED as int {
        if state == ST_CLOSED as int || state == ST_CRASHED as int {
            BUG_NONE as int
        } else {
            BUG_INVALID_TRANSITION as int
        }
    } else if event == EV_LOGIN_STARTED as int {
        if state == ST_LOGGED_OUT as int {
            BUG_NONE as int
        } else if state == ST_CLOSED as int {
            BUG_LOGIN_WITHOUT_OPEN as int
        } else {
            BUG_INVALID_TRANSITION as int
        }
    } else if event == EV_LOGIN_SUCCEEDED as int || event == EV_LOGIN_FAILED as int {
        if state == ST_LOGIN_PENDING as int {
            BUG_NONE as int
        } else {
            BUG_INVALID_TRANSITION as int
        }
    } else if event == EV_ASSESSMENT_STARTED as int {
        if state == ST_LOGGED_IN as int || state == ST_RESULT_VISIBLE as int {
            BUG_NONE as int
        } else {
            BUG_ASSESSMENT_WITHOUT_LOGIN as int
        }
    } else if event == EV_QUESTION_ANSWERED as int {
        if state == ST_ASSESSMENT as int {
            BUG_NONE as int
        } else {
            BUG_ANSWER_OUTSIDE_ASSESSMENT as int
        }
    } else if event == EV_ASSESSMENT_SUBMITTED as int {
        if state == ST_ASSESSMENT as int {
            BUG_NONE as int
        } else if state == ST_WAITING_RESULT as int {
            BUG_DUPLICATE_SUBMIT as int
        } else {
            BUG_SUBMIT_OUTSIDE_ASSESSMENT as int
        }
    } else if event == EV_RESULT_GENERATED as int {
        if state == ST_WAITING_RESULT as int {
            BUG_NONE as int
        } else {
            BUG_RESULT_WITHOUT_SUBMIT as int
        }
    } else if event == EV_RESULT_DISPLAYED as int {
        if state == ST_RESULT_READY as int {
            BUG_NONE as int
        } else {
            BUG_DISPLAY_WITHOUT_RESULT as int
        }
    } else if event == EV_NETWORK_LOST as int {
        if state == ST_CLOSED as int {
            BUG_INVALID_TRANSITION as int
        } else {
            BUG_NONE as int
        }
    } else if event == EV_NETWORK_RESTORED as int {
        if state == ST_OFFLINE as int {
            BUG_NONE as int
        } else {
            BUG_INVALID_TRANSITION as int
        }
    } else if event == EV_APP_BACKGROUNDED as int {
        if state == ST_CLOSED as int || state == ST_CRASHED as int {
            BUG_INVALID_TRANSITION as int
        } else {
            BUG_NONE as int
        }
    } else if event == EV_APP_FOREGROUNDED as int {
        if state == ST_BACKGROUND as int {
            BUG_NONE as int
        } else {
            BUG_INVALID_TRANSITION as int
        }
    } else if event == EV_APP_CLOSED as int {
        if state == ST_WAITING_RESULT as int || state == ST_LOGIN_PENDING as int {
            BUG_CLOSE_WHILE_PENDING as int
        } else {
            BUG_NONE as int
        }
    } else if event == EV_CRASH as int {
        if state == ST_CLOSED as int {
            BUG_INVALID_TRANSITION as int
        } else {
            BUG_NONE as int
...
// State after a valid event. For invalid events, keep the old state.
pub open spec fn next_state_spec(state: int, event: int) -> int {
    if transition_bug_spec(state, event) != BUG_NONE as int {
        state
    } else if event == EV_APP_OPENED as int {
        ST_LOGGED_OUT as int
    } else if event == EV_LOGIN_STARTED as int {
        ST_LOGIN_PENDING as int
    } else if event == EV_LOGIN_SUCCEEDED as int {
        ST_LOGGED_IN as int
    } else if event == EV_LOGIN_FAILED as int {
        ST_LOGGED_OUT as int
    } else if event == EV_ASSESSMENT_STARTED as int {
        ST_ASSESSMENT as int
    } else if event == EV_QUESTION_ANSWERED as int {
        ST_ASSESSMENT as int
    } else if event == EV_ASSESSMENT_SUBMITTED as int {
        ST_WAITING_RESULT as int
    } else if event == EV_RESULT_GENERATED as int {
        ST_RESULT_READY as int
    } else if event == EV_RESULT_DISPLAYED as int {
        ST_RESULT_VISIBLE as int
    } else if event == EV_NETWORK_LOST as int {
        ST_OFFLINE as int
    } else if event == EV_NETWORK_RESTORED as int {
        ST_LOGGED_OUT as int
    } else if event == EV_APP_BACKGROUNDED as int {
        ST_BACKGROUND as int
    } else if event == EV_APP_FOREGROUNDED as int {
        ST_LOGGED_OUT as int
    } else if event == EV_APP_CLOSED as int {
        ST_CLOSED as int
...
pub open spec fn trace_state_spec(events: Seq<u64>, n: nat) -> int
    recommends n <= events.len()
    decreases n
{
    if n == 0 {
        ST_CLOSED as int
    } else {
        let old_state = trace_state_spec(events, (n - 1) as nat);
        next_state_spec(old_state, events[(n - 1) as int] as int)
    }
}

pub open spec fn trace_first_bug_spec(events: Seq<u64>, n: nat) -> int
    recommends n <= events.len()
    decreases n
{
    if n == 0 {
        ...
        let previous_bug = trace_first_bug_spec(events, (n - 1) as nat);
        if previous_bug != BUG_NONE as int {
            previous_bug
        } else {
            let old_state = trace_state_spec(events, (n - 1) as nat);
            transition_bug_spec(old_state, events[(n - 1) as int] as int)
        }
    }
}

pub open spec fn trace_is_valid_spec(events: Seq<u64>) -> bool {
    events.len() > 0
    && trace_first_bug_spec(events, events.len()) == BUG_NONE as int
}

// ======================================================
// Executable verified transition checker
// ======================================================
...
pub fn severity_of_bug(v: u64) -> (s: u64)
    ensures s as int == violation_severity_spec(v as int)
{
    if v == BUG_NONE {
        SEV_NONE
    } else if v == BUG_UNKNOWN_EVENT {
        SEV_MEDIUM
    } else if v == BUG_INVALID_TRANSITION {
        SEV_HIGH
    } else if v == BUG_LOGIN_WITHOUT_OPEN {
        SEV_HIGH
    } else if v == BUG_ASSESSMENT_WITHOUT_LOGIN {
        SEV_HIGH
    } else if v == BUG_ANSWER_OUTSIDE_ASSESSMENT {
        SEV_MEDIUM
    } else if v == BUG_SUBMIT_OUTSIDE_ASSESSMENT {
        SEV_HIGH
    } else if v == BUG_DUPLICATE_SUBMIT {
        SEV_HIGH
    } else if v == BUG_RESULT_WITHOUT_SUBMIT {
        SEV_CRITICAL
    } else if v == BUG_DISPLAY_WITHOUT_RESULT {
        SEV_HIGH
    } else if v == BUG_EVENT_AFTER_CRASH {
        SEV_CRITICAL
    } else if v == BUG_CLOSE_WHILE_PENDING {
        SEV_MEDIUM
    } else {
        SEV_LOW
    }
...
pub fn transition_bug(state: u64, event: u64) -> (bug: u64)
    ensures bug as int == transition_bug_spec(state as int, event as int)
{
    if event < EV_APP_OPENED || event > EV_CRASH {
        BUG_UNKNOWN_EVENT
    } else if state == ST_CRASHED && event != EV_APP_OPENED {
        BUG_EVENT_AFTER_CRASH
    } else if event == EV_APP_OPENED {
        if state == ST_CLOSED || state == ST_CRASHED {
            BUG_NONE
        } else {
            BUG_INVALID_TRANSITION
        }
    } else if event == EV_LOGIN_STARTED {
        if state == ST_LOGGED_OUT {
            BUG_NONE
        } else if state == ST_CLOSED {
            BUG_LOGIN_WITHOUT_OPEN
        } else {
            BUG_INVALID_TRANSITION
        }
    } else if event == EV_LOGIN_SUCCEEDED || event == EV_LOGIN_FAILED {
        if state == ST_LOGIN_PENDING {
            BUG_NONE
        } else {
            BUG_INVALID_TRANSITION
        }
    } else if event == EV_ASSESSMENT_STARTED {
        if state == ST_LOGGED_IN || state == ST_RESULT_VISIBLE {
            BUG_NONE
        } else {
            BUG_ASSESSMENT_WITHOUT_LOGIN
        }
    } else if event == EV_QUESTION_ANSWERED {
        if state == ST_ASSESSMENT {
            BUG_NONE
        } else {
            BUG_ANSWER_OUTSIDE_ASSESSMENT
        }
    } else if event == EV_ASSESSMENT_SUBMITTED {
        if state == ST_ASSESSMENT {
            BUG_NONE
        } else if state == ST_WAITING_RESULT {
            BUG_DUPLICATE_SUBMIT
        } else {
            BUG_SUBMIT_OUTSIDE_ASSESSMENT
        }
    } else if event == EV_RESULT_GENERATED {
        if state == ST_WAITING_RESULT {
            BUG_NONE
        } else {
            BUG_RESULT_WITHOUT_SUBMIT
        }
    } else if event == EV_RESULT_DISPLAYED {
        if state == ST_RESULT_READY {
            BUG_NONE
        } else {
            BUG_DISPLAY_WITHOUT_RESULT
        }
    } else if event == EV_NETWORK_LOST {
        if state == ST_CLOSED {
            BUG_INVALID_TRANSITION
        } else {
            BUG_NONE
        }
    } else if event == EV_NETWORK_RESTORED {
        if state == ST_OFFLINE {
            BUG_NONE
        } else {
            BUG_INVALID_TRANSITION
        }
    } else if event == EV_APP_BACKGROUNDED {
        if state == ST_CLOSED || state == ST_CRASHED {
            BUG_INVALID_TRANSITION
        } else {
            BUG_NONE
        }
    } else if event == EV_APP_FOREGROUNDED {
        if state == ST_BACKGROUND {
            BUG_NONE
        } else {
            BUG_INVALID_TRANSITION
        }
    } else if event == EV_APP_CLOSED {
        if state == ST_WAITING_RESULT || state == ST_LOGIN_PENDING {
            BUG_CLOSE_WHILE_PENDING
        } else {
            BUG_NONE
        }
    } else if event == EV_CRASH {
        if state == ST_CLOSED {
            BUG_INVALID_TRANSITION
        } else {
            BUG_NONE
...
pub fn next_state(state: u64, event: u64) -> (next: u64)
    ensures next as int == next_state_spec(state as int, event as int)
{
    let bug = transition_bug(state, event);
    ...
    if bug != BUG_NONE {
        ...
        ST_LOGIN_PENDING
    } else if event == EV_LOGIN_SUCCEEDED {
        ST_LOGGED_IN
    } else if event == EV_LOGIN_FAILED {
        ST_LOGGED_OUT
    } else if event == EV_ASSESSMENT_STARTED {
        ST_ASSESSMENT
    } else if event == EV_QUESTION_ANSWERED {
...
pub fn check_transition(state: u64, event: u64) -> (r: TransitionResult)
    ensures
        r.bug as int == transition_bug_spec(state as int, event as int),
        r.next_state as int == next_state_spec(state as int, event as int),
        r.severity as int == violation_severity_spec(r.bug as int)
{
    let bug = transition_bug(state, event);
    let next = next_state(state, event);
    let severity = severity_of_bug(bug);
...
pub fn check_app_trace(events: &Vec<u64>) -> (report: TraceReport)
    ensures
        report.processed == events.len(),
        events.len() == 0 ==> !report.valid,
        report.valid ==> report.first_bug == BUG_NONE,
        report.first_bug != BUG_NONE ==> !report.valid
{
    if events.len() == 0 {
        return TraceReport {
            valid: false,
            final_state: ST_CLOSED,
            first_bug: BUG_EMPTY_TRACE,
            severity: SEV_LOW,
            bug_index: 0,
            processed: 0,
        };
    ...
    let mut state: u64 = ST_CLOSED;
    let mut first_bug: u64 = BUG_NONE;
    let mut first_severity: u64 = SEV_NONE;
    let mut bug_index: usize = events.len();
    let mut i: usize = 0;

    while i < events.len()
        invariant
            i <= events.len(),
            first_bug == BUG_NONE || bug_index < i,
            first_bug == BUG_NONE ==> first_severity == SEV_NONE,
            first_bug != BUG_NONE ==> first_severity as int
                == violation_severity_spec(first_bug as int)
        decreases events.len() - i
    {
        let event = events[i];
        let step = check_transition(state, event);

        if first_bug == BUG_NONE && step.bug != BUG_NONE {
            first_bug = step.bug;
            first_severity = step.severity;
            bug_index = i;
        }
...
// ======================================================
// K-Sentry + behavior checker combined result
// ======================================================

pub struct ProtectedTraceReport {
    pub integrity_tag: u64,
    pub behavior_valid: bool,
    pub first_bug: u64,
    pub severity: u64,
    pub bug_index: usize,
    pub final_state: u64,
...
pub fn protect_and_check_trace(
    events: Vec<u64>,
    q: u64,
    p: u64,
    k: u64
) -> (out: ProtectedTraceReport)
    requires
        events.len() as int <= k as int,
        k <= 4294967295u64,
        p > 1,
        q >= 1
    ensures
        out.integrity_tag as int
            == ramanujan_spec(events.view(), q as int, p as int),
        out.integrity_tag < p
{
    let tag = run_from_empty_k(events.clone(), q, p, k);
    let behavior = check_app_trace(&events);

    ProtectedTraceReport {
        integrity_tag: tag,
        behavior_valid: behavior.valid,
        first_bug: behavior.first_bug,
        severity: behavior.severity,
        bug_index: behavior.bug_index,
        final_state: behavior.final_state,
    }
}

// ======================================================
// Safe adversarial trace mutations
// These mutate only supplied test vectors.
// ======================================================

pub fn attack_delete_event(trace: &Vec<u64>, index: usize) -> (out: Vec<u64>)
    requires
        index < trace.len()
{
...
pub fn attack_duplicate_event(trace: &Vec<u64>, index: usize) -> (out: Vec<u64>)
    requires
        index < trace.len()
{
    let mut out = Vec::<u64>::new();
    ...
    while i < trace.len()
        invariant
            i <= trace.len(),
            out.len() == i + if i > index { 1usize } else { 0usize }
        decreases trace.len() - i
    {
        out.push(trace[i]);
...
pub fn attack_swap_adjacent(trace: &Vec<u64>, index: usize) -> (out: Vec<u64>)
    requires
        trace.len() >= 2,
        index < trace.len() - 1
{
    ...
    {
        if i == index {
            assert(index + 1 < trace.len());
            out.push(trace[index + 1]);
        } else if i > 0 && i - 1 == index {
            assert(index < trace.len());
            out.push(trace[index]);
        } else {
            out.push(trace[i]);
        }
        i += 1;
...
pub fn attack_truncate(trace: &Vec<u64>, new_len: usize) -> (out: Vec<u64>)
    requires new_len <= trace.len()
    ensures out.len() == new_len
{
    let mut out = Vec::<u64>::new();
    let mut i: usize = 0;
...
pub struct AttackCheck {
    ...
    pub attacked_tag: u64,
    pub integrity_changed: bool,
    pub original_behavior_valid: bool,
    pub attacked_behavior_valid: bool,
    pub attacked_bug: u64,
    pub attacked_severity: u64,
    pub attacked_bug_index: usize,
}

pub fn compare_original_and_attacked(
    original: Vec<u64>,
    attacked: Vec<u64>,
    q: u64,
    p: u64,
    ...
    requires
        original.len() as int <= k as int,
        attacked.len() as int <= k as int,
        k <= 4294967295u64,
        p > 1,
        q >= 1
{
    let original_report = protect_and_check_trace(original, q, p, k);
    let attacked_report = protect_and_check_trace(attacked, q, p, k);

    AttackCheck {
        original_tag: original_report.integrity_tag,
        attacked_tag: attacked_report.integrity_tag,
        integrity_changed:
            original_report.integrity_tag != attacked_report.integrity_tag,
        original_behavior_valid: original_report.behavior_valid,
        attacked_behavior_valid: attacked_report.behavior_valid,
        attacked_bug: attacked_report.first_bug,
        attacked_severity: attacked_report.severity,
        attacked_bug_index: attacked_report.bug_index,
    }
}

// ======================================================
// Small proof-facing theorems
// ======================================================

pub proof fn theorem_duplicate_submit_is_rejected()
    ensures
        transition_bug_spec(
            ST_WAITING_RESULT as int,
            EV_ASSESSMENT_SUBMITTED as int
        ) == BUG_DUPLICATE_SUBMIT as int
{
}

pub proof fn theorem_result_without_submit_is_rejected()
    ensures
        transition_bug_spec(
            ST_LOGGED_IN as int,
            EV_RESULT_GENERATED as int
        ) == BUG_RESULT_WITHOUT_SUBMIT as int
{
}

pub proof fn theorem_assessment_requires_login()
    ensures
        transition_bug_spec(
            ST_LOGGED_OUT as int,
            EV_ASSESSMENT_STARTED as int
        ) == BUG_ASSESSMENT_WITHOUT_LOGIN as int
{
}

pub proof fn theorem_valid_happy_path_transitions()
    ensures
        transition_bug_spec(ST_CLOSED as int, EV_APP_OPENED as int)
            == BUG_NONE as int,
        transition_bug_spec(ST_LOGGED_OUT as int, EV_LOGIN_STARTED as int)
            == BUG_NONE as int,
        transition_bug_spec(ST_LOGIN_PENDING as int, EV_LOGIN_SUCCEEDED as int)
            == BUG_NONE as int,
        transition_bug_spec(ST_LOGGED_IN as int, EV_ASSESSMENT_STARTED as int)
            == BUG_NONE as int,
        transition_bug_spec(ST_ASSESSMENT as int, EV_QUESTION_ANSWERED as int)
            == BUG_NONE as int,
        transition_bug_spec(ST_ASSESSMENT as int, EV_ASSESSMENT_SUBMITTED as int)
            == BUG_NONE as int,
        transition_bug_spec(ST_WAITING_RESULT as int, EV_RESULT_GENERATED as int)
            == BUG_NONE as int,
        transition_bug_spec(ST_RESULT_READY as int, EV_RESULT_DISPLAYED as int)
            == BUG_NONE as int
{
}

} // verus!
fn print_report(name: &str, report: &TraceReport) {
    println!("==============================");
    println!("{}", name);
    println!("==============================");
    println!("Valid behavior: {}", report.valid);
    println!("Final state: {}", report.final_state);
    println!("First bug: {}", report.first_bug);
    println!("Severity: {}", report.severity);
    println!("Bug index: {}", report.bug_index);
    println!("Events processed: {}", report.processed);
    println!();
}

fn print_attack(name: &str, report: &AttackCheck) {
    println!("==============================");
    println!("{}", name);
    println!("==============================");
    println!("Original tag: {}", report.original_tag);
    println!("Attacked tag: {}", report.attacked_tag);
    println!("Integrity changed: {}", report.integrity_changed);
    println!(
        "Original behavior valid: {}",
        report.original_behavior_valid
    );
    println!(
        "Attacked behavior valid: {}",
        report.attacked_behavior_valid
    );
    println!("Attacked bug: {}", report.attacked_bug);
    println!("Attacked severity: {}", report.attacked_severity);
    println!("Attacked bug index: {}", report.attacked_bug_index);
    println!();
}
...
fn main() {
    ...
    // ----------------------------------
    // Valid application trace
    // ----------------------------------

    let valid_trace: Vec<u64> = vec![
        EV_APP_OPENED,
        EV_LOGIN_STARTED,
        EV_LOGIN_SUCCEEDED,
        EV_ASSESSMENT_STARTED,
        EV_QUESTION_ANSWERED,
        EV_ASSESSMENT_SUBMITTED,
        EV_RESULT_GENERATED,
        EV_RESULT_DISPLAYED,
    ];

    let valid_report = check_app_trace(&valid_trace);
    print_report("VALID TRACE", &valid_report);

    // ----------------------------------
    // Invalid trace: assessment before login
    // ----------------------------------

    let no_login_trace: Vec<u64> = vec![
        EV_APP_OPENED,
        EV_ASSESSMENT_STARTED,
        EV_QUESTION_ANSWERED,
        EV_ASSESSMENT_SUBMITTED,
    ...
    let no_login_report = check_app_trace(&no_login_trace);
    print_report("ASSESSMENT WITHOUT LOGIN", &no_login_report);

    // ----------------------------------
    // Invalid trace: duplicate submission
    // ----------------------------------

    let duplicate_submit_trace: Vec<u64> = vec![
        EV_APP_OPENED,
        EV_LOGIN_STARTED,
        EV_LOGIN_SUCCEEDED,
        EV_ASSESSMENT_STARTED,
    ...
    let duplicate_submit_report =
        check_app_trace(&duplicate_submit_trace);

    print_report(
        "DUPLICATE ASSESSMENT SUBMISSION",
        &duplicate_submit_report,
    );

    // ----------------------------------
    // Invalid trace: result before submission
    // ----------------------------------

    let early_result_trace: Vec<u64> = vec![
        EV_APP_OPENED,
        EV_LOGIN_STARTED,
        EV_LOGIN_SUCCEEDED,
        EV_RESULT_GENERATED,
    ...
    let early_result_report = check_app_trace(&early_result_trace);
    print_report("RESULT WITHOUT SUBMISSION", &early_result_report);

    // ----------------------------------
    // Invalid trace: event after crash
    // ----------------------------------

    let crash_trace: Vec<u64> = vec![
        EV_APP_OPENED,
        EV_LOGIN_STARTED,
        EV_LOGIN_SUCCEEDED,
        EV_CRASH,
    ...
    let crash_report = check_app_trace(&crash_trace);
    print_report("EVENT AFTER CRASH", &crash_report);

    // ----------------------------------
    // Attack: delete login success
    // Original index 2 is EV_LOGIN_SUCCEEDED
    // ----------------------------------

    let deleted_trace = attack_delete_event(&valid_trace, 2);

    let delete_attack = compare_original_and_attacked(
        valid_trace.clone(),
        deleted_trace,
        q,
        p,
    ...
    print_attack("DELETE EVENT ATTACK", &delete_attack);

    // ----------------------------------
    // Attack: duplicate submission
    // Original index 5 is submission
    // ----------------------------------

    let duplicated_trace =
        attack_duplicate_event(&valid_trace, 5);

    let duplicate_attack = compare_original_and_attacked(
        valid_trace.clone(),
        duplicated_trace,
        q,
        p,
    ...
    print_attack("DUPLICATE EVENT ATTACK", &duplicate_attack);

    // ----------------------------------
    // Attack: replace login success
    // Replace it with result-generated event
    // ----------------------------------

    let replaced_trace = attack_replace_event(
        &valid_trace,
        2,
        EV_RESULT_GENERATED,
    );

    let replace_attack = compare_original_and_attacked(
        valid_trace.clone(),
        replaced_trace,
        q,
        p,
    ...
    print_attack("REPLACE EVENT ATTACK", &replace_attack);

    // ----------------------------------
    // Attack: swap submission and result
    // ----------------------------------

    let swapped_trace =
        attack_swap_adjacent(&valid_trace, 5);

    let swap_attack = compare_original_and_attacked(
        valid_trace.clone(),
        swapped_trace,
        q,
        p,
    ...
    print_attack("REORDER EVENT ATTACK", &swap_attack);

    // ----------------------------------
    // Attack: truncate final events
    // ----------------------------------

    let truncated_trace =
        attack_truncate(&valid_trace, 5);

    let truncate_attack = compare_original_and_attacked(
        valid_trace.clone(),
        truncated_trace,
        q,
        p,
    ...
    print_attack("TRUNCATION ATTACK", &truncate_attack);

    println!("K-Sentry demonstration completed.");
}
